...
- Windows / Ubuntu 员工端：`v0.1.7`
- Codex Broker 服务端：`v0.1.7`
...
- **员工自助领 Token**：管理员按“名字首字母.姓氏”（例如 `YX.GUO`）和工号增加用户，员工通过内网入口验证后查询自己的设备令牌；服务器不保存明文密码或令牌。
...
| 文件 | 用途 |
| --- | --- |
| `CWS-Codex-Release-v0.1.7.zip` | Windows 推荐分发包，包含图形安装器和简明使用说明 |
| `CWS-Codex-Setup-v0.1.7.exe` | Windows 员工端安装器，启动时可从 GitHub Release 自动更新 |
| `CWS-Codex-Windows-v0.1.7.zip` | Windows 脚本版及故障排查备用包 |
| `CWS-Codex-Linux-v0.1.7.tar.gz` | Ubuntu/Linux 员工端，支持图形更新提示 |
| `CWS-Codex-Server-v0.1.7.tar.gz` | Codex Broker 服务端部署包，支持模型容量健康探测、服务器优先客户端更新，并兼容旧客户端 |
