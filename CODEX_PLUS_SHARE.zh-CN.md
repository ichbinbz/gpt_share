...
Windows 员工日常使用建议分发 `dist/CWS-Codex-Release-v0.1.7.zip`，其中包含标准图形安装器
`CWS-Codex-Setup-v0.1.7.exe` 和简明的 `使用说明.txt`。员工解压后双击安装器即可安装，并可在
Windows“已安装的应用”中卸载。`dist/CWS-Codex-Windows-v0.1.7.zip`
保留用于管理员排障。员工输入管理员
...
Ubuntu/Linux 员工端发布包为 `dist/CWS-Codex-Linux-v0.1.7.tar.gz`。员工解压后以普通用户运行
...
员工令牌若错误地安装到多台电脑，用量会合并到该令牌名下。员工端只上传散列后的会话 ID、模型
Token 计数，以及在本机计算的用户消息数、字符数和“用户文本 Token（估算）”；不上传提示词、
回复、代码或文件内容。服务端对同一令牌、同一会话和同一天保存最大累计值，重复上报不会重复
计数，并按北京时间自然周（周一 00:00）、自然月（每月 1 日 00:00）和累计展示。v0.1.3 旧客户端
仍可同步凭据和上报累计 Token，但升级到 v0.1.4 后才会提供用户输入与周/月分项。
