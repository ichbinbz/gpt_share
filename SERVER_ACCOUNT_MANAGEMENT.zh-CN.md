...
页面底部的“用户与 Token 自助”区域可以增加员工用户：用户名填写“名字首字母.姓氏”的大写格式，
例如 `YX.GUO`，工号同时作为初始密码。用户名和工号均不可重复。增加后，员工通过内网代理/VLESS 打开：

```text
http://codex.cws.internal:8765/token
```

输入上述用户名和工号即可查询自己的设备令牌。服务器使用 PBKDF2 保存密码哈希，仅保存设备令牌的
SHA-256 摘要，不保存两者明文。`CWS_CODEX_USER_TOKEN_SECRET` 用于确定性重建令牌，创建用户后
必须保持稳定；建议与管理令牌使用两个不同的随机长值。查询接口会对连续失败登录进行限速。
