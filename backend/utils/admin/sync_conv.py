    ...
    async with get_async_session_context() as session:
        ...
        for conv_db in existed_conversations:
            ...
            if result:
                ...
                browser_account_id = same_one_in_openai_server.get("_cws_browser_account_id")
                # 同步标题
                ...
                # 更新 source_id
                if browser_account_id and conv_db.source_id != browser_account_id:
                    conv_db.source_id = browser_account_id
                    logger.info(
                        f"Conversation {conv_db.conversation_id} assigned to browser account {browser_account_id}"
                    )
                elif is_team and conv_db.source_id is None:
                ...
                if conv_db.is_valid and config.openai_web.transport != "browser":
                    # A browser pool sync may be partial when one account is offline.
        ...
        for same_one_in_openai_server, is_team in merged_conversations.values():
            ...
            new_conv = OpenaiWebConversation(
                ...
                source_id=browser_account_id or (config.openai_web.team_account_id if is_team else None)
            )
