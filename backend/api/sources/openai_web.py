...
class OpenaiWebChatManager(metaclass=SingletonMeta):
    def __init__(self):
        ...
        self.browser_pool = ChatGPTBrowserPool(config.openai_web)
        self.reset_session()

    @property
    def browser_bridge(self):
        """Backward-compatible access to the default browser bridge."""
        return self.browser_pool.default_bridge

    @property
    def use_browser_transport(self) -> bool:
        return config.openai_web.transport == "browser"

    async def startup(self):
        if self.use_browser_transport and config.openai_web.enabled:
            await self.browser_pool.startup()

    async def shutdown(self):
        await self.browser_pool.shutdown()

    async def reload_browser_pool(self):
        old_pool = self.browser_pool
        self.browser_pool = ChatGPTBrowserPool(config.openai_web)
        self.semaphore = asyncio.Semaphore(config.openai_web.max_completion_concurrency)
    ...
    def reset_session(self):
        if hasattr(self, "browser_pool"):
            self.browser_pool.mark_stale()
        self.session = make_session()

    async def check_accounts(self) -> OpenaiWebAccountsCheckResponse:
        if self.use_browser_transport:
            result = await self.browser_pool.default_bridge.same_origin_fetch(
                "/backend-api/accounts/check/v4-2023-04-27"
            )
            return OpenaiWebAccountsCheckResponse(**result)
    ...
    async def get_conversations(self, timeout=None, use_team: bool = False) -> list[dict]:
        if self.use_browser_transport:
            ...
            for account in self.browser_pool.all_accounts():
                ...
                try:
                    while True:
                        data = await account.bridge.same_origin_fetch(
                            f"/backend-api/conversations?offset={offset}&limit={limit}"
                        )
                        conversations = data.get("items", [])
                        for conversation in conversations:
                            conversation["_cws_browser_account_id"] = account.account_id
                    ...
                    logger.warning(
                        f"Cannot list conversations for browser account {account.account_id}: {exc}"
                    )
            if successful_accounts == 0:
                raise OpenaiWebException("No ChatGPT browser account could list conversations", code=503)
            return results
        if timeout is None:
    ...
    async def get_conversation_history(self, conversation_id: uuid.UUID | str,
        ...
        if self.use_browser_transport:
            result = await self.browser_pool.account(source_id).bridge.same_origin_fetch(
    ...
    async def clear_conversations(self, use_team: bool = False):
        if self.use_browser_transport:
            await asyncio.gather(
                *(account.bridge.same_origin_fetch(
                    "/backend-api/conversations", method="PATCH", body={"is_visible": False}
                ) for account in self.browser_pool.all_accounts() if account.enabled),
    ...
    async def complete(self, model: OpenaiWebChatModels, text_content: str, use_team: bool,
        ...
        if self.use_browser_transport:
            if plugin_ids or attachments or multimodal_image_parts:
                raise InvalidParamsException(
                    "Browser transport currently supports text chat only; plugins and uploads must be disabled"
                )
            model_name = model.value if model else OpenaiWebChatModels.chatgpt_auto.value
            async with self.browser_pool.lease(
                preferred_account_id=browser_account_id,
                model_name=model_name,
            ) as account:
                async for event in account.bridge.complete(
                    model_name=model_name,
                    model_slug=(model or OpenaiWebChatModels.chatgpt_auto).code(),
                    text_content=text_content,
                    conversation_id=str(conversation_id) if conversation_id else None,
                    parent_message_id=str(parent_message_id) if parent_message_id else None,
                ):
                    event["_cws_browser_account_id"] = account.account_id
                    yield event
            return
    ...
    async def delete_conversation(self, conversation_id: str, source_id: str = None):
        if self.use_browser_transport:
            await self.browser_pool.account(source_id).bridge.same_origin_fetch(
                f"/backend-api/conversation/{conversation_id}",
                method="PATCH",
    ...
    async def generate_conversation_title(self, conversation_id: str, message_id: str, use_team: bool):
        if self.use_browser_transport:
            source_id = use_team if isinstance(use_team, str) else None
            result = await self.browser_pool.account(source_id).bridge.same_origin_fetch(
    ...
    async def get_installed_plugin_manifests(self, offset=0, limit=250,
        ...
        if self.use_browser_transport:
            return OpenaiChatPluginListResponse(items=[], count=0)
        params = {
            "offset": offset,
            "limit": limit,
            "is_installed": True,
    ...
    async def change_plugin_user_settings(self, plugin_id: str, setting: OpenaiChatPluginUserSettings,
                                          use_team: bool):
        if self.use_browser_transport:
            raise InvalidParamsException("Legacy ChatGPT plugins are not supported by browser transport")
        if not config.openai_web.is_plus_account:
            raise InvalidParamsException("errors.notPlusChatgptAccount")
    ...
    async def get_interpreter_info(self, conversation_id: str, source_id: str | None):
        if self.use_browser_transport:
            return await self.browser_pool.account(source_id).bridge.same_origin_fetch(
                f"/backend-api/conversation/{conversation_id}/interpreter"
            )
        response = await self.session.get(
    ...
    async def get_file_download_url(self, file_id: str, use_team: bool):
        if self.use_browser_transport:
            result = await self.browser_bridge.same_origin_fetch(
                f"/backend-api/files/{file_id}/download"
            )
            if result.get("status") == "success":
                return result.get("download_url")
            raise ResourceNotFoundException(f"Failed to get download URL for {file_id}")
        response = await self.session.get(
    ...
    async def get_interpreter_file_download_url(self, conversation_id: str, message_id: str, sandbox_path: str,
        ...
        if self.use_browser_transport:
            ...
            result = await self.browser_pool.account(source_id).bridge.same_origin_fetch(
                f"/backend-api/conversation/{conversation_id}/interpreter/download?"
                + urlencode({"message_id": message_id, "sandbox_path": sandbox_path})
            )
            if result.get("status") == "success":
                return result.get("download_url")
            raise ResourceNotFoundException("Failed to get interpreter download URL")
        response = await self.session.get(
    ...
    async def get_file_upload_url(self, upload_info: OpenaiChatFileUploadUrlRequest,
        ...
        if self.use_browser_transport:
            result = await self.browser_bridge.same_origin_fetch(
                "/backend-api/files", method="POST", body=upload_info.model_dump()
            )
            return OpenaiChatFileUploadUrlResponse.model_validate(result)
        """
    ...
    async def check_file_uploaded(self, file_id: str, use_team: bool) -> str:
        ...
        if self.use_browser_transport:
            result = await self.browser_bridge.same_origin_fetch(
                f"/backend-api/files/{file_id}/uploaded", method="POST", body={}
            )
            if result.get("status") == "success":
                return result.get("download_url")
            raise OpenaiWebException(f"Failed to check uploaded file {file_id}")
