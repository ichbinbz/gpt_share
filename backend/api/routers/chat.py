...
async def chat(websocket: WebSocket):
    ...
    if not ask_request.new_conversation:
        ...
        # 是否可用 team 对话
        if conversation is not None and conversation.source_id is not None and use_team == False and \
                not openai_web_manager.use_browser_transport:
    ...
    # 在此之前应当没有任何副作用
    message = None
    selected_browser_account_id = conversation.source_id if conversation is not None else None
    ...
    try:
        ...
        async for data in manager.complete(model=model,
                                           ...
                                           browser_account_id=selected_browser_account_id,
                                           ):
            has_got_reply = True

            if ask_request.source == ChatSourceTypes.openai_web:
                selected_browser_account_id = data.get(
                    "_cws_browser_account_id", selected_browser_account_id
                )
        ...
        async with get_async_session_context() as session:
            ...
            if ask_request.new_conversation:
                ...
                if ask_request.source == ChatSourceTypes.openai_web and ask_request.new_title is not None and \
                    ...
                    try:
                        source_id = selected_browser_account_id
                        if source_id is None and use_team:
                ...
                if selected_browser_account_id is not None:
                    new_conv.source_id = selected_browser_account_id
                elif use_team:
