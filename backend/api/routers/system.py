...
from api.sources.codex_broker_status import get_codex_broker_status, get_codex_device_usage
...
async def update_config(config_model: ConfigModel, _user: User = Depends(current_super_user)):
    ...
    await openai_web_manager.reload_browser_pool()
    return config.model()
...
@router.get("/system/check-openai-web-browser", tags=["system"])
async def check_openai_web_browser(_user: User = Depends(current_super_user)):
    """Report whether the configured CDP browser and ChatGPT login are ready."""
    openai_web_manager = OpenaiWebChatManager()
    if not openai_web_manager.use_browser_transport:
        return {"transport": "legacy_http", "connected": False, "logged_in": False}
    result = await openai_web_manager.browser_bridge.health()
    return {"transport": "browser", **result}


@router.get("/system/openai-web-browser-pool", tags=["system"])
async def get_openai_web_browser_pool(
        refresh: bool = False,
        refresh_quota: bool = False,
        _user: User = Depends(current_super_user)):
    """Return live health, load and quota estimates for every browser account."""
    openai_web_manager = OpenaiWebChatManager()
    if not openai_web_manager.use_browser_transport:
        return {"transport": "legacy_http", "capacity": 0, "busy": 0, "accounts": []}
    return await openai_web_manager.browser_pool.status(
        refresh=refresh,
        refresh_quota=refresh_quota,
    )


@router.get("/system/codex-account-quotas", tags=["system"])
async def get_codex_account_quotas(
        refresh: bool = False,
        _user: User = Depends(current_super_user)):
    """Return a credential-free Codex account quota summary to CWS administrators."""
    return await get_codex_broker_status(refresh=refresh)


@router.get("/system/codex-device-usage", tags=["system"])
async def get_codex_employee_usage(_user: User = Depends(current_super_user)):
    """Return per-employee Codex token totals attributed by device token."""
    return await get_codex_device_usage()
