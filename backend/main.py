...
async def startup():
    ...
    await g.chatgpt_manager.startup()
    ...
    if config.openai_web.transport == "legacy_http" and config.openai_web.chatgpt_base_url is None:
        logger.error("chatgpt_base_url is not set in config!")
        exit(1)
    elif config.openai_web.transport == "browser":
        logger.info(
            f"Using ChatGPT browser transport via {config.openai_web.browser_cdp_url}"
        )
    else:
        logger.info(
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        if g.chatgpt_manager is not None:
            await g.chatgpt_manager.shutdown()
